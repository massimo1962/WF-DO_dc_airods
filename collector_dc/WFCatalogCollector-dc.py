"""
WFCatalog Collector Dublin Core Metadata 
...
Modifier:
(dc_on) 
  @ Massimo Fares (massimo.fares@ingv.it) 2017


[CONFIG]
  Option description from config2.json (modify on config.json)
  ...
  "DUBLIN_CORE":{
    "ENABLED": true,
    "CLS_ORM": field needed for orm mongo usd in http-api-airods i.e."b2stage.models.mongo.wf_do",
    "AUTH_NETWORKS": network id to process i.e. ["IV"],
    "STATION_ENDPOINT" : fdsn ws endpoint i.e. "http://webservices.rm.ingv.it/fdsnws/station/1/query?",
    "HTTP_CONNECTION" : url of fdsn endpoint i.e. "webservices.rm.ingv.it"
  },
  "IRODS": {
    "ENABLED": true,
    "HOST": Host of irods
...
[USAGE]
This script allow insert into MongoDB collection dc_on all Dublin Core metadata retrieved from station xml, irods PID and other stuff.
...
[FLAGS]
  ...
  ### dublin core flag
  [--dc_on] enable ingestion DC metadata into MongoDB 
...
# Load configuration from JSON2
# config2.json instead of config.json - EIDA
cfg_dir = os.path.dirname(os.path.realpath(__file__))
with open(os.path.join(cfg_dir, 'config2.json'), "r") as cfg:
...
class WFCatalogCollector():
  ...
  def getDataStations(self):
    ...
    self.mystations = {}
    mynet = CONFIG['DUBLIN_CORE']['AUTH_NETWORKS']
    
    query = CONFIG['DUBLIN_CORE']['STATION_ENDPOINT']+"network="+mynet+"&format=text"
    conn = http.client.HTTPConnection(CONFIG['DUBLIN_CORE']['HTTP_CONNECTION'], 80)
    conn.connect()
  ...
  def process(self, options):
    ...
    print("start processing at: "+str(self.timeInitialized))
  ...
  def _processFilesDO(self):
    """
    WFCatalogCollector._processFilesDO
    > Loop over all files added to the dublin_core mongo collection 
    """
  	# Load stations info
    self.getDataStations() 

  	# Make iRODS connection  	
    paramHost = CONFIG['IRODS']['HOST'] 
    paramPort = CONFIG['IRODS']['PORT']
    paramUser = CONFIG['IRODS']['USER']
    paramPswd = 'sdor' #CONFIG['IRODS']['PSWD']
    paramZone = CONFIG['IRODS']['ZONE']
    
    self.session = iRODSSession(host=paramHost, port=paramPort, user=paramUser, password=paramPswd, zone=paramZone)    
    #self.session = iRODSSession(host="irods2.int.ingv.it", port="1247", user="rods", password="sdor", zone="INGV")
    ...
    for file in self.files:
        ...
        try:
            ...
            if my_doc is not None:                
                self.mongo._storeFileDataObject(my_doc)
            ...
            self.log.error("Could not compute metadata irods")
            self.log.error(ex)
        continue
  ...
  def _createDataObject(self, file):
    ...
    """
    # connect to irods:
    currentCursor  = self.mongo.getFileDataObject(file)


    if currentCursor.count() != 0:
    	self.log.info(file+" :: file already present!")
    	return None
    # retrieve file info (like PID)    
    # irods base path
    irodsPathBase = CONFIG['IRODS']['BASE_PATH'] #"/INGV/home/rods/san/archive/"
    ...
    # Channel
    cha = os.path.basename(file).split('.')[3]+".D"
    
    # Year
    year = os.path.basename(file).split('.')[5]
    
    # iRDOS Path
    irodsPath = irodsPathBase+year+"/"+net+"/"+sta+"/"+cha+"/"+os.path.basename(file)
    print (irodsPath)
 
    try:
        obj = self.session.data_objects.get(irodsPath)
        ...
        PID = '11099/fakePid'
        print ("PID missing on: "+os.path.basename(file)+" : insert fake pid")
	pass
	#raise
    
    
    #Lat-lon-elevation
    #print (self.mystations[sta])
    georef = self.mystations[sta]  
    elevation= 0
    #Time-window
